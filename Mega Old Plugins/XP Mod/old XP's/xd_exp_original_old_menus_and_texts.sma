...
public dodatki_menu(id)
{
	...
	new msg1[128]
	format(msg1,127,"\wHow to \rbuy \yVIP \w[EN]")
	menu_additem(dodatki_menu  , msg1, "1", 0)
	new msg2[128]
	format(msg2,127,"\wKak da \rkupite \yVIP \w[BG]")
...
public xp_menu(id)
{
	...
	new msg1[128]
	format(msg1,127,"\yHead \rAward \wMenu")
	...
	new msg7[128]
	format(msg7,127,"\yVisit \wsite: \yhttp\w://\rxd\w-\rgaming\w.\yinfo^n")
...
public eDeath() { 
	...
	if(get_user_team(attacker) != get_user_team(victim) && attacker != 0) {
		...
		if (get_user_flags(attacker) & ADMIN_RESERVATION) {
			if(PlayerXP[attacker] < PlayerXP[victim]){
				if(headshot) {
					...
					ColorChat(attacker, RED, "^x04[xD eXp Mod]^x01 You got more^x03 XP^x01. You^x03 killed^x01 with^x03 HEAD^x04SHOT^x01.");
					//UpdateHUD(attacker);
					SaveData(attacker); 
				}
				else 
				{
					...
					PlayerXP[attacker] += 5;
					PlayerXP[attacker] += 10;
					ColorChat(attacker, RED, "^x04[xD eXp Mod]^x01 You got more^x03 XP^x01. Your^x03 kill^x01 is^x03 Normal^x04!!!");
					//UpdateHUD(attacker);
					SaveData(attacker); 
				}
			}
		...
		while(PlayerXP[attacker] >= LEVELS[PlayerLevel[attacker]]) { 
			ColorChat(attacker, RED, "^x04[xD eXperience Mod]^x01 Congratulations! XP^x03 %i^x01 level, type^x03 /menu^x01 to take^x03 advantage of a^x04 point^x01.", PlayerLevel[attacker + 1]);
...
public exp_wybierz_2(id, menu_exp_2 , item)
{
	...
	{ 
		...
		case 3: { 
			if(PlayerLevel[id] <= 14) {
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need^x03 20^x01 level!");
			}
			else if(punkty[id] == 0) {
				ColorChat(id, RED, "^x04[xD eXperience Mod]^x01 Need^x03 points^x01 for this^x04!!!");
			}
...
public EventRoundEnd(id) {
	for (new id=0; id < 32; id++) {
		...
		if(is_user_alive(id) && get_user_team(id) == 1) { // && get_user_team == 1
			ColorChat(id, RED, "^x04[xD eXperience Mod]^x01 You got^x03 more^x04 exp^x01 for the^x03 survival^x01 of the^x04 round^x01!!!");
			PlayerXP[id] += 5;
		}
	}
}
