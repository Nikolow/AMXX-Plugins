...
public plugin_init() { 
	...
	//Cvary
	gCvar_Enable    = register_cvar("emu_save", "1"); 
	gCvar_Kill    = register_cvar("emu_kill", "30"); 
	gCvar_HS    = register_cvar("emu_hs", "40"); 
	g_Vault        = nvault_open("exm_hns"); 
	
	register_concmd("emu_removexp", "cmd_take_exp", ADMIN_IMMUNITY, "<target> <amount>");  
	register_concmd("emu_addxp", "cmd_give_exp", ADMIN_IMMUNITY, "<target> <amount>"); 
	register_concmd("emu_removeptk", "cmd_take_ptk", ADMIN_IMMUNITY, "<target> <amount>");  
	register_concmd("emu_addptk", "cmd_give_ptk", ADMIN_IMMUNITY, "<target> <amount>"); 
	...
	//register_clcmd("say /menu", "skill_1");  			// skill 1
	//register_clcmd("say_team /menu", "skill_1");  		// skill 1
	//register_clcmd("say /menu", "skill_2");  			// skill 2
	//register_clcmd("say_team /menu", "skill_2");  		// skill 2
	//register_clcmd("say /menu", "skill_3");  			// skill 3
	///register_clcmd("say_team /menu", "skill_3");  		// skill 3
	...
	//HeadSplash
	amx_headsplash = register_cvar("emu_headsplash", "1"); // Register the on/off cvar.
...
public resetModel(id, level, cid) {
	if (get_user_flags(id) & ADMIN_RESERVATION) {
		...
		if (userTeam == CS_TEAM_T) {
			cs_set_user_model(id, "emu_vip_tt");
		}
		else if(userTeam == CS_TEAM_CT) {
			cs_set_user_model(id, "emu_vip_ct");
		}
		else {
...
public dodatki_menu(id)
{
	...
	new msg1[128]
	format(msg1,127,"\wHow to buy VIP [EN]")
	menu_additem(dodatki_menu  , msg1, "1", 0)
	new msg2[128]
	format(msg2,127,"\wKak da kupite VIP [BG]")
	menu_additem(dodatki_menu  , msg2, "2", 0)
	new msg3[128]
	format(msg3,127,"\wAdmin Awards | Nagradi [BG]")
	menu_additem(dodatki_menu  , msg3, "3", 0)
	//opcja wyjœcia "Wyjœcie" zamiast "Exit"
	menu_setprop(dodatki_menu,MPROP_EXITNAME,"\wExit");
	...
	//menu_setprop(menu_exp,MPROP_NUMBER_COLOR,"\y");
	//menu_setprop(menu_exp,MPROP_TITLE,"\rJestem fajny");
...
public dodatki_menu_wybierz(id, dodatki_menu  , item)
{
	...
	{ 
		...
		case 3 : {
			show_motd (id,"vip_info_3.txt","Admin Nagradi || Award's   [BG <> EN]")
		}
...
public xp_menu(id)
{
	...
	new msg1[128]
	format(msg1,127,"\wHead Award Menu")
	menu_additem(xp_menu  , msg1, "1", 0)
	new msg2[128]
	format(msg2,127,"\wReset Your Rank and Level [XP]^n")
	menu_additem(xp_menu  , msg2, "2", 0)
	new msg3[128]
	format(msg3,127,"\wPlayer Information")
	menu_additem(xp_menu  , msg3, "3", 0)
	new msg4[128]
	format(msg4,127,"\wVip Information")
	menu_additem(xp_menu  , msg4, "4", 0)
	new msg5[128]
	format(msg5,127,"\wGold Vip Access^n")
	menu_additem(xp_menu  , msg5, "5", 0)
	new msg6[128]
	format(msg6,127,"\wChat Information Commands")
	menu_additem(xp_menu  , msg6, "6", 0)
	new msg7[128]
	format(msg7,127,"\wVisit site: http://xd-gaming.info^n")
	menu_additem(xp_menu  , msg7, "7", 0)
	new msg8[128]
	format(msg8,127,"\wDisconnect server [leave]")
	menu_additem(xp_menu  , msg8, "8", 0)
	new msg9[128]
	format(msg9,127,"\wStop play [quit]^n")
	menu_additem(xp_menu  , msg9, "9", 0)
	new msg10[128]
	format(msg10,127,"\wExit")
	menu_additem(xp_menu  , msg10, "0", 0)
	//opcja wyjœcia "Wyjœcie" zamiast "Exit"
	//menu_setprop(menu_exp,MPROP_EXITNAME,"\wWyjscie");
...
public xp_menu_wybierz(id, xp_menu  , item)
{
	...
	{ 
		...
		case 7 : {
			ColorChat(id, RED, "^x01*^x04Web^x03Site^x01 http://^x03xD^x01-^x04GaminG^x01.^x03info" );
		}
		case 8: {
...
public reset_napewno(id)
{
	...
	new msg1[128]
	format(msg1,127,"\wYes \d(Do you want realy reset Rank and Level ?)")
	menu_additem(menu_reset , msg1, "1", 0)
	new msg2[128]
	format(msg2,127,"\wNo")
	menu_additem(menu_reset , msg2, "2", 0)
	//opcja wyjœcia "Wyjœcie" zamiast "Exit"
	//menu_setprop(menu_exp,MPROP_EXITNAME,"\wExit");
...
public reset_wybierz(id, menu_reset , item)
{
	...
	{ 
		case 1 : {
			reset_punkty(id);
			ColorChat(id, GREEN, "******** Reset successfully!");
			ColorChat(id, GREEN, "******** You need %d", PlayerLevel[id]);
		}
		case 2 : {
			//reset_punkty(id);
			//ColorChat(id, GREEN, "******** Adding points > xp successfully!");
		}
...
public reset_punkty(id) {
	...
	//PlayerXP[id] = 0;  // nie zabieramy expa
	//PlayerLevel[id] = 0;  // nie zabieramy levela
...
public reklama(id) {
	ColorChat(id, RED, "^x04[xD eXperience Mod]^x01 This server^x04 using^x03 [xD eXperiance Mod]^x01 by^x03 xD^x01-^x04GaminG",VERSION);
}
...
public eDeath() { 
	...
	if(get_user_team(attacker) != get_user_team(victim) && attacker != 0) {
		...
		if (get_user_flags(attacker) & ADMIN_RESERVATION) {
			if(PlayerXP[attacker] < PlayerXP[victim]){
				if(headshot) {
					...
					ColorChat(attacker, RED, "^x04[xD eXperience Mod]^x03 '10 xp'^x01 for^x04 normal^x03 kill^x01, '15 xp'^x01 for^x04 HS^x03 kill^x01.");
					//UpdateHUD(attacker);
					SaveData(attacker); 
				}
				else 
				{
					...
					PlayerXP[attacker] += 5;
					PlayerXP[attacker] += 10;
					//client_print(attacker, print_chat, "[XP] Dostales 20 expa za zabicie gracza.
					ColorChat(attacker, RED, "^x04[xD eXperience Mod]^x03 '10 xp'^x01 for^x04 normal^x03 kill^x01, '15 xp'^x01 for^x04 HS^x03 kill^x01.");
					//UpdateHUD(attacker);
					SaveData(attacker); 
				}
		...
		//PlayerXP[attacker] += get_pcvar_num(gCvar_Kill); 
		//client_print(attacker, print_chat, "[XP] Dostales 20 expa za zabicie gracza."); 
		//ColorChat(attacker, GREEN, "[emu eXperience Mod]^x01 Dostales^03 5^x01 expa za zabicie gracza.");
		
		//if (get_user_flags(attacker) & ADMIN_RESERVATION) {
		//	PlayerXP[attacker] += 5; 
		//client_print(attacker, print_chat, "[XP] Dostales 20 expa za zabicie gracza.
		//	ColorChat(attacker, GREEN, "[emu eXperience Mod]^x01 Dostales Bonus^03 5^x01 expa za to ze jestes VIP'em!");
		//UpdateHUD(attacker);
		//}
		
		//if(headshot) {
		//	PlayerXP[attacker] += get_pcvar_num(gCvar_HS); 
		//	ColorChat(attacker, GREEN, "[emu eXperience Mod]^x01 Dostales Bonus^03 5^x01 za HeadShoot'a");
		//UpdateHUD(attacker);
		//}
		...
		while(PlayerXP[attacker] >= LEVELS[PlayerLevel[attacker]]) { 
			//client_print(attacker, print_chat, "[XP] Gratulacje! Wbiles %i level, napisz /xp aby wykorzystac punkt.", PlayerLevel[attacker]); 
			ColorChat(attacker, GREEN, "[xD eXperience Mod]^x01 Congratulations! XP^x03 %i^x01 level, type^x03 /menu^x01 to take advantage of a point.", PlayerLevel[attacker + 1]);
...
// skill 1 - niewidzialnosc
public skill_1(id) {
	if(punktysk1[id] <= 0) {
	}
	else if(punktysk1[id] <= 0) {
		ColorChat(id, GREEN, "[xD eXperience Mod]^x01 You have add^x03 2^x01 points!");
	}
	else if(uzyl[id] == true) {
		ColorChat(id, GREEN, "[xD eXperience Mod]^x03 Skill^x01 for 1 round!");
	}
	else
	{
		set_user_rendering(id, kRenderFxGlowShell, 0, 0, 0, kRenderTransColor, 0);
		set_task(6.0, "skill_1_off",id,"",0,"a", 1);
		ColorChat(id, RED, "^x04[xD eXperience Mod]^x03 Invisible^x01 for^x04 6^x01 seconds !");
		uzyl[id] = true
	}
}

public skill_1_off(id) {
	set_user_rendering(id, kRenderFxGlowShell, 0, 0, 0, kRenderNormal, 255);
	ColorChat(id, RED, "^x04[xD eXperience Mod]^x03 Now is normal !!!");
}
...
public skill_2(id) {
	...
	else
	{
		if (get_user_team(id) == 1) // 1 - terro
		{
			...
			set_task(15.0, "skill_2_off",id,"",0,"a", 1);
			ColorChat(id, RED, "^x04[xD eXperience Mod]^x01 Camoflauge^x03 [CT]^x01 for^x04 15^x01 seconds! > > You look like^x03 CT for^x04 15^x01 seconds!");
			uzyl[id] = true
		}
		else
		{
			...
			set_task(15.0, "skill_2_off",id,"",0,"a", 1);
			ColorChat(id, RED, "^x04[xD eXperience Mod]^x01 Camoflauge^x03 [TT]^x01 for^x04 15^x01 seconds! > > You look like^x03 TT for^x04 15^x01 seconds!");
			uzyl[id] = true
		}
	}
}

public skill_2_off(id) {
	cs_reset_user_model(id);
	ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Now look NORMAL");
}
...
public skill_3(id) {
	...
	else
	{
		...
		set_task(5.0, "skill_3_off",id,"",0,"a", 1)
		ColorChat(id, RED, "^x04[xD eXperience Mod]^x01 You have^x03 400^x04 gravity^x01 for^x03 5^x01 seconds!");
		uzyl[id] = true
	}
}

public skill_3_off(id) {
	set_user_gravity(id, 1.0)
	ColorChat(id, RED, "^x04[xD eXperience Mod]^x03 Gravity is NORMAL");
}
	...
	for (new id=0; id < 32; id++) {
		...
		if (is_user_alive(id))
		{
			if(PlayerLevel[ id ] >= 15) {
				set_hudmessage(223,116,1,0.01, 0.18,0,1.0, 12.0)
				show_hudmessage(id, "Level: Max / 15^nExp: Max / 18100 (+0)^nRank: EasyBlock GOD");
			}
			else 
			{
				set_hudmessage(223,116,1,0.01, 0.18,0,1.0, 12.0)
				show_hudmessage(id, "Level: %i / 15^nExp: %d / %d (+%d)^nRank: %s",PlayerLevel[id],PlayerXP[id],LEVELS[PlayerLevel[id]], LEVELS[PlayerLevel[id]] - PlayerXP[id], rank[id]);
			}
			//set_hudmessage(255, 0, 0, 0.01, 0.90, 0, 6.0, 12.0)
			//format(msghud1,171,"Poziom: %d^nExp: %d / %d",PlayerLevel[id],PlayerXP[id],LEVELS[PlayerLevel[id]+1])
			//show_hudmessage(id,msghud1)
			
			//new text[512];
			
			//format(text,511,"Poziom: %i / 15 Exp: %d / %d (+%d) Ranga: %s",PlayerLevel[id],PlayerXP[id],LEVELS[PlayerLevel[id]], LEVELS[PlayerLevel[id]] - PlayerXP[id], rank[id]);
...
{
	new msg0[128]
	format(msg0,127,"\w[\rxD eXperience Mod\w]\y Head\r 1\w/\r2^n\w[\yPoints > XP:\r %d\w]",punkty[id])
	new menu_exp = menu_create(msg0, "exp_wybierz")
	new msg1[128]
	format(msg1,127,"\wHP \r[\w%d\y/\w5\r] \d(2 lvl)",punktyhp[id])
	menu_additem(menu_exp , msg1, "1", 0)
	new msg2[128]
	format(msg2,127,"\wArmor \r[\w%d\y/\w5\r]",punktyarm[id])
	menu_additem(menu_exp , msg2, "2", 0)
	new msg3[128]
	format(msg3,127,"\wRespawn \r[\w%d\y/\w1\r] \d(10 lvl)",punktyrespawn[id])
	menu_additem(menu_exp , msg3, "3", 0)
	new msg4[128]
	format(msg4,127,"\wInvisible \r[\w%d\y/\w1\r] \d(8 lvl)",punktysk1[id])
	menu_additem(menu_exp , msg4, "4", 0)
	new msg5[128]
	format(msg5,127,"\wCamouflage \r[\w%d\y/\w1\r] \d(5 lvl)",punktysk2[id])
	menu_additem(menu_exp , msg5, "5", 0)
	new msg6[128]
	format(msg6,127,"\wGravity \r[\w%d\y/\w1\r] \d(15 lvl)",punktysk3[id])
	menu_additem(menu_exp , msg6, "6", 0)
	new msg7[128]
	format(msg7,127,"\wWeapons \r[\w%d\y/\w2\r]",bronie[id])
	menu_additem(menu_exp , msg7, "7", 0)
	new msg8[128]
	format(msg8,127,"\wStrengthening \r[\w%d\y/\w10\r]^n",punktywzmoc[id])
	menu_additem(menu_exp , msg8, "8", 0)
	new msg9[128]
	format(msg9,127,"\wNext\y (page 2)")
...
{
	new msg0[128]
	format(msg0,127,"\w[\rxD eXperience Mod\w]\y Head\r 2\w/\r2^n\w[\yPoints > XP:\r %d\w]",punkty[id])
	new menu_exp_2 = menu_create(msg0, "exp_wybierz_2")
	new msg1[128]
	format(msg1,127,"\wWalk = no sound \r[\w%d\y/\w1\r]",punktychodzenie[id])
	menu_additem(menu_exp_2 , msg1, "1", 0)
	new msg2[128]
	format(msg2,127,"\wGrenade \r[\w%d\y/\w2\r]",punktygranat[id])
	menu_additem(menu_exp_2 , msg2, "2", 0)
	new msg3[128]
	format(msg3,127,"\wPostol \r[\w%d\y/\w5\r] \d(15 lvl)",punktypistolety[id])
	menu_additem(menu_exp_2 , msg3, "3", 0)
	new msg4[128]
	format(msg4,127,"\wRifles \r[\w%d\y/\w5\r] \d(15 lvl)",punktykarabiny[id])
	menu_additem(menu_exp_2 , msg4, "4", 0)
	new msg5[128]
	format(msg5,127,"\wSet \r[\w%d\y/\w5\r] \d(15 lvl)^n^n^n^n",punktyzestaw[id])
...
public exp_wybierz(id, menu_exp , item)
{
	...
	{ 
		case 1 : { 
			if(PlayerLevel[id] <= 1) {
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need^x03 2^x01 levela!");
			}
			else if(punkty[id] == 0) {
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need points!");
			}
			else 
			{
				if(punktyhp[id] >= 5) {
					ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Now you have^x03 %i^x04 /^x03 3^x01!", punktyhp[id]);
				}
				else 
				{
					punkty[id] -= 1;
					punktyhp[id] += 1;
					ColorChat(id, GREEN, "******** All is successfully!");
				}
			}
		...
		case 2 :  { 
			...
			{
				if(punktyarm[id] >= 5) {
					ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Sorry, you already have^x03 %i^x04 /^x03 3^x01!", punktyarm[id]);
				}
				else 
				{
					punkty[id] -= 1;
		...
		case 3 :  { 
			if(PlayerLevel[id] <= 9) {
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need^x03 10^x01 levela!");
			}
			else if(punkty[id]<= 0) {
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need points!");
			}
		...
		case 4 :  { 
			if(PlayerLevel[id] <= 7) {
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need^x03 8^x01 levela!");
			}
			...
			else 
			{
				...
				else 
				{
					...
					punktysk1[id] += 1;
					skille[id] += 1;
					ColorChat(id, GREEN, "******** Adding points to the invisible skill is completed successfully!");
				}
			}
			menu_destroy(menu_exp); 
			menu_xp(id);
		...
		case 5 :  { 
			if(PlayerLevel[id] <= 4) {
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need^x03 5^x01 levela!");
			}
			else if(punkty[id] <= 1) { // else if
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need points!");
			}
			...
			else 
			{
				...
				else 
				{
					...
					punktysk2[id] += 1;
					skille[id] += 1;
					ColorChat(id, GREEN, "******** Adding points to the skill Kammuflaz completed successfully!");
				}
			}
			menu_destroy(menu_exp); 
			menu_xp(id); 
		...
		case 6 :  { 
			if(PlayerLevel[id] <= 14) {
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need^x03 15^x01 levela!");
			}
			else if(punkty[id] <= 1) { // else if
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need points!");
			}
			...
			else 
			{
				...
				else 
				{
					...
					punktysk3[id] += 1;
					skille[id] += 1;
					ColorChat(id, GREEN, "******** Adding points to the skill Gravitation completed successfully!");
				}
			}
			menu_destroy(menu_exp);
			menu_xp(id);
		...
		case 7: { 
			...
			else 
			{
				...
				else 
				{
					punkty[id] -= 1;
					bronie[id] += 1;
					ColorChat(id, GREEN, "******** Adding points to Arms is completed successfully!");
				}
			}
			menu_destroy(menu_exp); 
			menu_xp(id); 
		...
		case 8: { 
			...
			else 
			{
				if(punktywzmoc[id] >= 10) {
					ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Sorry, you already have^x03 %i^x04 /^x03 5^x01!", punktywzmoc[id]);
				}
				else 
				{
					punkty[id] -= 1;
					punktywzmoc[id] += 1;
					ColorChat(id, GREEN, "******** Adding points to the reinforcement is completed with success!");
				}
			}
			menu_destroy(menu_exp); 
			menu_xp(id); 
...
public exp_wybierz_2(id, menu_exp_2 , item)
{
	...
	{ 
		case 1: { 
			...
			else 
			{
				...
				else 
				{
					...
					ColorChat(id, GREEN, "******** Adding points to the Quiet Walk is completed successfully!");
				}
			}
		...
		case 2: { 
			...
			else 
			{
				...
				else 
				{
					punkty[id] -= 1;
					punktygranat[id] += 1;
					ColorChat(id, GREEN, "******** Adding points to grenades is completed successfully!");
				}
			}
			menu_destroy(menu_exp_2); 
			menu_xp_2(id); 
		...
		case 3: { 
			if(PlayerLevel[id] <= 14) {
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need^x03 20^x01 levela!");
			}
			else if(punkty[id] == 0) {
				ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Need points");
			}
			else 
			{
				...
				else 
				{
					...
					ColorChat(id, GREEN, "******** Adding points for guns is completed successfully!");
				}
			}
			menu_destroy(menu_exp_2); 
			menu_xp_2(id); 
	...
	if(los <= 7 * bronie[id]){ // 7 * bronie[id]
		if(punktypistolety[id] >= 1) {
			...
			switch(bronie_los)  {
				case 1: {
					...
					ColorChat(id, GREEN, "[%d '/.]^x01 Got Glock with 4 bullets!", 7 * bronie[id]);
				}
				case 2: {
					give_item( id, "weapon_fiveseven" );
					cs_set_user_bpammo(id, CSW_FIVESEVEN, 0);
					cs_set_weapon_ammo( find_ent_by_owner( 3, "weapon_fiveseven", id ), 3); 
					ColorChat(id, GREEN, "[%d '/.]^x01 Got FiveSeven 3 bullets!", 7 * bronie[id]);
				}
			}
		}
		else 
		{
			...
			switch(bronie_los)  {
				case 1: {
					...
					ColorChat(id, GREEN, "[%d '/.]^x01 Got a pop gun with a bullet!", 7 * bronie[id]);
				}
				case 2: {
					give_item( id, "weapon_m3" );
					cs_set_user_bpammo(id, CSW_M3, 0);
					cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_m3", id ), 1); 
					ColorChat(id, GREEN, "[%d '/.]^x01 Got to shoot a bullet!", 7 * bronie[id]);
				}
				case 3: {
					give_item( id, "weapon_tmp" );
					cs_set_user_bpammo(id, CSW_TMP, 0);
					cs_set_weapon_ammo( find_ent_by_owner( 2, "weapon_tmp", id ), 2); 
					ColorChat(id, GREEN, "[%d '/.]^x01 Got Wiertare with 2 bullets!", 7 * bronie[id]);
		...
		switch(bronie_los)  {
			case 1: {
				...
				ColorChat(id, GREEN, "[%d '/.]^x01 Got Uzi with a bullet!", 10 * bronie[id]);
			}
			case 2: {
				give_item( id, "weapon_mp5navy" );
				cs_set_user_bpammo(id, CSW_MP5NAVY, 0);
				cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_mp5navy", id ), 1); 
				ColorChat(id, GREEN, "[%d '/.]^x01 Got MP5Navy with 1 bullet!", 10 * bronie[id]);
			}
			case 3: {
				give_item( id, "weapon_glock18" );
				cs_set_user_bpammo(id, CSW_GLOCK18, 0);
				cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_glock18", id ), 1); 
				ColorChat(id, GREEN, "[%d '/.]^x01 Got Glock with a bullet!", 10 * bronie[id]);
			...
			case 4: {
				...
				ColorChat(id, GREEN, "[%d '/.]^x01 Usp got from a bullet!", 10 * bronie[id]);
		...
		switch(bronie_los)  {
			case 1: {
				...
				ColorChat(id, GREEN, "[%d '/.]^x01 Got to shoot a bullet!", 8 * bronie[id]);
			}
			case 2: {
				give_item( id, "weapon_fiveseven" );
				cs_set_user_bpammo(id, CSW_FIVESEVEN, 0);
				cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_fiveseven", id ), 1); 
				ColorChat(id, GREEN, "[%d '/.]^x01 Got FiveSeven with 1 bullet!", 8 * bronie[id]);
			}
			case 3: {
				give_item( id, "weapon_elite" );
				cs_set_user_bpammo(id, CSW_ELITE, 0);
				cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_elite", id ), 1); 
				ColorChat(id, GREEN, "[%d '/.]^x01 Got Elite with 1 bullet!", 8 * bronie[id]);
		...
		if(punktyzestaw[id] >= 1){
			...
			switch(bronie_los)  {
				case 1: {
					...
					cs_set_user_bpammo(id, CSW_GLOCK18, 0);
					cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_glock18", id ), 1); 
					ColorChat(id, GREEN, "[%d '/.]^x01 Got a kit with a bullet!", 3 * bronie[id]);
				}
				case 2: {
			...
			switch(bronie_los)  {
				case 1: {
					...
					cs_set_user_bpammo(id, CSW_M4A1, 0);
					cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_m4a1", id ), 1);  
					ColorChat(id, GREEN, "[%d '/.]^x01 Got M4A1 with a bullet!", 3 * bronie[id]);
				}
				case 2: {
					give_item( id, "weapon_ak47" );
					cs_set_user_bpammo(id, CSW_AK47, 0);
					cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_ak47", id ), 1);  
					ColorChat(id, GREEN, "[%d '/.]^x01 Got Ak-47 with a bullet!", 3 * bronie[id]);
				}
		...
		else 
		{
			...
			switch(bronie_los)  {
				case 1: {
					...
					ColorChat(id, GREEN, "[%d '/.]^x01 DGot Awp with 1 bullet!", 3 * bronie[id]);
				}
				case 2: {
					give_item( id, "weapon_deagle" );
					cs_set_user_bpammo(id, CSW_DEAGLE, 0);
					cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_deagle", id ), 1); 
					ColorChat(id, GREEN, "[%d '/.]^x01 Got Deagle with one bullet!", 3 * bronie[id]);
				}
				case 3: {
					give_item( id, "weapon_tmp" );
					cs_set_user_bpammo(id, CSW_TMP, 0);
					cs_set_weapon_ammo( find_ent_by_owner( 2, "weapon_tmp", id ), 2); 
					ColorChat(id, GREEN, "[%d '/.]^x01 Got Wiertare with 2 bullets!", 3 * bronie[id]);
		...
		ColorChat(id, GREEN, "******** I'm sorry, you got nothing.");
	}
}
public weapon_granaty(id) {
	...
	if(los <= 50 * punktygranat[id]){
		if (get_user_team(id) == 1) // 1 - terro
		{
			...
			give_item(id, "weapon_flashbang"); 
			ColorChat(id, GREEN, "[%d '/.]^x01 Got a set of grenades!", 50 * punktygranat[id]);
		}
		else
		{
			//give_item(id, "weapon_smokegrenade"); 
			...
			//give_item(id, "weapon_flashbang"); 
			//give_item(id, "weapon_flashbang"); 
			ColorChat(id, GREEN, "[%d '/.]^x01 Explosive Grenade!", 50 * punktygranat[id]);
		}
		
	}
	
	else { // 49 procent ze nic nie wypadnie
		//ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Bad luck .. you got nothing.");
	}
}
public EventRoundEnd(id) {
	for (new id=0; id < 32; id++) {
		...
		if(is_user_alive(id) && get_user_team(id) == 1) { // && get_user_team == 1
			ColorChat(id, GREEN, "[xD eXperience Mod]^x01 Got^x03 5^x01 exp for the survival of the round!");
			PlayerXP[id] += 5;
		}
	}
}
...
public cmd_give_exp( id, level,cid ) { 
	...
	switch( get_cvar_num ( "amx_show_activity" ) ) { 
		case 1: client_print( 0, print_chat, "ADMIN: Ohh. Player %s got %i exp.", expnum, player_name ); 
			case 2: client_print( 0, print_chat, "ADMIN %s distance %i exp for %s.", admin_name, expnum, player_name ); 
		} 
	
	client_print( player, print_chat, "[xD eXperience Mod] Have gained %i exp (Total: %d)", expnum, PlayerXP[player] ); 
	SaveData( id ); 
...
public cmd_take_exp( id, level,cid ) { 
	...
	client_print( player, print_chat, "[xD eXperience Mod] Lost %i exp (Total: %d)", expnum, PlayerXP[player] ); 
	SaveData( id ); 
	
	return PLUGIN_CONTINUE; 
}  
// Give/Take PTK addaon by [X]-RayCat, emu
public cmd_give_ptk( id, level,cid ) { 
	...
	switch( get_cvar_num ( "amx_show_activity" ) ) { 
		case 1: client_print( 0, print_chat, "ADMIN: Ohh. Player %s got %i exp.", expnum, player_name ); 
			case 2: client_print( 0, print_chat, "ADMIN %s distance %i exp Player %s.", admin_name, expnum, player_name ); 
		} 
	
	client_print( player, print_chat, "[xD eXperience Mod] Have gained %i points (Total: %i)", expnum, punkty[player] ); 
	SaveData( id ); 
...
public cmd_take_ptk( id, level,cid ) { 
	...
	client_print( player, print_chat, "[xD eXperience Mod] Lost %i points (Total: %i)", expnum, punkty[player] ); 
	SaveData( id ); 
...
public respawn(id) {
	...
	switch(losik)  {
		...
		case 1: {
			...
			ColorChat(id, GREEN, "[xD eXperience Mod]^x01 You have been zrespiony");
			EventRoundStart(id);
		}
		case 2:  {
...
public damage_player(pwned, pwnzor, Float:damage) // Damages or kills a player. Home made HAX
{
	...
	else
	{
		...
		//PlayerXP[pwnzor] += 7;
		//ColorChat(pwnzor, GREEN, "[xD eXperience Mod]^x01 Get^03 7^x01 exp for killing a player head_splashem");
		
		//PlayerXP[victim] -= 7;
		//ColorChat(attacker, GREEN, "[EXP MOD]^x01 Lost^03 7^x01 exp for killing a player head_splashem.");
		...
		if (get_user_flags(pwnzor) && ADMIN_RESERVATION) {
			if(PlayerXP[pwnzor] < PlayerXP[pwned]){
				...
				PlayerXP[pwnzor] += 10;
				PlayerXP[pwnzor] += 5;
				ColorChat(pwnzor, GREEN, "[xD eXperience Mod]^x01 Got^03 17^x01 exp for killing a player head_splashem and with higher Levels.");
				//UpdateHUD(attacker);
			}
			else 
			{
				...
				PlayerXP[pwnzor] += 5;
				//client_print(attacker, print_chat, "[XP] Dostales 20 expa za zabicie gracza.
				ColorChat(pwnzor, GREEN, "[xD eXperience Mod]^x01 Got^03 7^x01 exp for killing a player head_splashem.");
				//UpdateHUD(attacker);
			}
		}
		...
		while(PlayerXP[pwnzor] >= LEVELS[PlayerLevel[pwnzor]]) { 
			//client_print(attacker, print_chat, "[XP] Congratulations! Wbiles %i level, napisz /xp aby wykorzystac punkt.", PlayerLevel[attacker]); 
			ColorChat(pwnzor, GREEN, "[EXP MOD]^x01 Congratulations! You pocket^x03 %i^x01 level, type^x03 /menu^x01 to take advantage of the point.", PlayerLevel[pwnzor + 1]);
		...
		if(get_user_team(pwnzor) != get_user_team(pwned)) // If it was a team kill, the pwnzor's money should get reduced instead of increased.
		{
			...
			//PlayerXP[victim] += 20;
			//ColorChat(attacker, GREEN, "[EXP MOD]^x01 Dostales^03 20^x01 expa za zabicie gracza head_splashem.");
...
if(victim != attacker)
{
	...
	if( read_data(3) )
	{
		// headshot 
		PlayerXP[attacker] += get_pcvar_num(gCvar_HS); 
		ColorChat(attacker, GREEN, "[EXP MOD]^x01 Got^03 20^x01 exp for killing a player in the head.");
	}
	
	ColorChat(attacker, GREEN, "[EXP MOD]^x01 Got^03 20^x01 exp for killing a player.");
	
	while(PlayerXP[attacker] >= LEVELS[PlayerLevel[attacker]]) { 
		//client_print(attacker, print_chat, "[XP] Gratulacje! Wbiles %i level, napisz /xp aby wykorzystac punkt.", PlayerLevel[attacker]); 
		ColorChat(attacker, GREEN, "[EXP MOD]^x01 Congratulations! You pocket^x03 %i^x01 level, type^x03 /xp^x01 to take advantage of the point.", PlayerLevel[attacker]);
		PlayerLevel[attacker] += 1; 
		punkty[attacker] += 1;
	} 
...
new bronie_los = random_num(0,100)
...
case 0..5: {
	...
	cs_set_user_bpammo(id, CSW_AWP, 0);
	cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_awp", id ), 1); 
	ColorChat(id, GREEN, "[emu eXperience Mod]^x01 Dostales^x03 Awp^x01 z^x03 1^x01 nabojem (^x03szansa:^x03 5%^x01)");
}
case 0..10: {
	give_item( id, "weapon_deagle" );
	cs_set_user_bpammo(id, CSW_DEAGLE, 0);
	cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_deagle", id ), 1); 
	ColorChat(id, GREEN, "[emu eXperience Mod]^x01 Dostales^x03 Deagle^x01 z^x03 1^x01 nabojem (^x03szansa:^x03 10%^x01)");
}
case 0..20: {
	give_item( id, "weapon_elite" );
	cs_set_user_bpammo(id, CSW_ELITE, 0);
	cs_set_weapon_ammo( find_ent_by_owner( 1, "weapon_elite", id ), 1); 
	ColorChat(id, GREEN, "[emu eXperience Mod]^x01 Dostales^x03 Elitki^x01 z^x03 1^x01 nabojem (^x03szansa:^x03 20%^x01)");
