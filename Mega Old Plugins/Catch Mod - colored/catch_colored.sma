...
public plugin_init() {
	...
	// ===================================================================================Cvars
	register_cvar("Catch_version","2.0.1",FCVAR_SERVER)
	
	register_cvar("catch_speed","2.0")		// catch_speed
	register_cvar("catch_f_speed","1.0")		// normalspeed * catch_speed * catch_f_speed
	register_cvar("catch_o_speed","1.0")		// normalspeed * catch_speed * catch_o_speed
	register_cvar("catch_l_speed","1.15")		// normalspeed * catch_speed * catch_l_speed
	register_cvar("catch_t_speed","1.5")		// speed * catch_t_speed
	register_cvar("catch_showteam","1")		// shows wich team
	register_cvar("catch_godmode","1")		// Godmode
	register_cvar("catch_distancecheck","0")	// 4 players with bad commands
	register_cvar("catch_noknifes","0")
	register_cvar("catch_distance","70")		// DistanceCheck
	register_cvar("catch_nofootsteps","0")		// no Sounds, by running
	register_cvar("catch_render","1")		// not change. ever 1.^^
	register_cvar("catch_bhop","1")		// bunnyhopp
	register_cvar("catch_autobhop","0")		// Automatish Bunnyhop
	register_cvar("catch_boost","700")		// Boost
	register_cvar("catch_boostmode","0")		// Boostmode (0 = old | 1 = new)
	register_cvar("catch_turbo","1")		// Turbo
...
public speedup(id) {
	...
	if(team[id-1] == 1)
		speed = 320.0 * get_cvar_float("catch_speed") * get_cvar_float("catch_f_speed")
	else {
		if(other_num() == 1 && !wait && get_playersnum() > 2)
			speed = 320.0 * get_cvar_float("catch_speed") * get_cvar_float("catch_l_speed")
		else
			speed = 320.0 * get_cvar_float("catch_speed") * get_cvar_float("catch_o_speed")
	}

	if(get_cvar_num("catch_turbo") && turbo[id-1][0] == 1)
		speed *= get_cvar_float("catch_t_speed")
...
public render(id) {
	if(get_cvar_num("catch_render") == 1) {
		if(team[id-1] == 0)
...
public catch_on(id) {
	if(enable == true)
		ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 The Mod is^x03 active !")
	else {
		enable = true
		vote = false
		...
		for(new i=1;i<=get_maxplayers();i++)
			if(is_user_connected(i)) {
				ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 The Mod is^x03 active !")
				ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 Say^x03 '!stats'^x01 for^x04 see^x01 the^x03 Statesboard^x01 and^x03 '!help'^x01 if you^x04 need^x03 HELP!")
...
public catch_off(id,level,cid) {
	...
	if(enable == false)
		ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 The Mod is^x03 deactive!")
	else {
		enable = false
		remove_task(1000)
		remove_task(6000)
...
public catch_stats(id) {
	if(enable) {
		ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 You^x03 catched^x04 %d^x01 and would be^x03 %d catched.",score[id-1][0],score[id-1][2])
		ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 You^x03 survived^x04 %d^x04 Rounds^x01.^x03 Lucker",score[id-1][3])
	}
...
public turbo_on(id) {
	if(is_user_alive(id) && team[id-1] == 0 && get_cvar_num("catch_turbo")) {
		if(turbo[id-1][1] < TURBO_VERBRAUCH) {
			turbo[id-1][0] = 0
			speedup(id)
		}
...
public turbo_task(id) {
	...
	if(is_user_alive(pid) && is_user_connected(pid)) {
		if(enable) {
			if(team[pid-1] == 0 && turbo[pid-1][0] == 1 && get_cvar_num("catch_turbo")) {
...
public distance_check() {
	if(get_cvar_num("catch_distancecheck") == 1 && !wait) {
		for(new i=1;i<33;i++) {
			if(is_user_alive(i)) {
				for(new x=1;x<33;x++) {
					if(is_user_alive(x) && is_visible(x,i) && i != x && team[i-1] != team[x-1]) {
						...
						if(get_distance(iOrigin,xOrigin) <= get_cvar_num("catch_distance")) {
...
public resethud(id) {
	if(enable) {
		...
		if(firstspawn[id-1]) {
			ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 Welcome,^x03 [H]ave [F]un^x01 &^x04 [G]ood [L]uck !")
			ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 Say^x03 '!stats'^x01 for^x04 see^x01 the^x03 Statesboard^x01 and^x03 '!help'^x01 if you^x04 need^x03 HELP!")
		}
	}
...
public apply(id) {
	...
	if(get_cvar_num("catch_godmode") == 1)
		set_user_godmode(id,1)
		
	if(get_cvar_num("catch_nofootsteps") == 1)
	...
	if(team[id-1] == 1)
		ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 You are now a^x03 CATCHER.^x04 You have to catch.^x03 Go,Go,Go...")
	else
		ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 You have now to^x03 FLEE^x01,^x04Take care...")
...
public client_PreThink(id) {
	if(enable) {
		...
		if(get_cvar_num("catch_bhop") == 1 && buttons|IN_DUCK)
			entity_set_float(id,EV_FL_fuser2,0.1)
		
		if(get_cvar_num("catch_autobhop") == 1) {
			if(buttons & IN_JUMP) {
		...
		new clip, ammo
		if(get_cvar_num("catch_noknifes") == 0 && user_has_weapon(id,29) && get_user_weapon(id,clip,ammo) != 29) {
			client_cmd(id,"use weapon_knife")
		}
		
		if(get_cvar_num("catch_turbo")) {
			...
			if(is_user_alive(id) && is_user_alive(aimid) && id != aimid && aimid > 0 && aimid < 33 && team[id-1] == team[aimid-1] && viewangles[0] < -75.0) {
				...
				if(get_distance(pOrigin,aOrigin) <= MIN_BOOST_DIST) {
					if(buttons & IN_ATTACK) {
						new Float:velocity[3]
						if(get_cvar_num("catch_boostmode") == 1)
...
public endround() {
	if(enable && !blockround) {
		...
		if(other_win == 1) {
			for(new i=1;i<33;i++) {
				if(is_user_connected(i))
					ColorChat(i,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 Fleers^x03 won this^x04 round^x01.^x03 +%d^x04 Point%s",punkte*UEBERLEBEN,punkte*UEBERLEBEN > 1 ? "e" : "")
			}
		}
		else {
			for(new i=1;i<33;i++) {
				if(is_user_connected(i))
					ColorChat(i,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 Catchers^x03 won this^x04 round!")
			}	
		}
	...
	if(vote == true) {
		if(key == 0) {
			...
			if(get_cvar_num("amx_vote_answers") == 1) {
				...
				for(new i=1;i<=get_maxplayers();i++)
					if(is_user_connected(i))
						ColorChat(id,GREY,"^x04[^x03 Catch VOTE^x04 ]^x01 %s^x03 voted for^x04 Yeah...",name)
			}
		}
		else if(key == 1) {
			vote_erg[id-1] = false
			...
			if(get_cvar_num("amx_vote_answers") == 1) {
				...
				for(new i=1;i<=get_maxplayers();i++)
					if(is_user_connected(i))
						ColorChat(i,GREY,"^x04[^x03 Catch VOTE^x04 ]^x01 %s^x03 voted for^x04 Never",name)
			}
		}
	}
}
...
public check_vote(id) {
	if(vote) {
		...
		for(new i=1;i<=get_maxplayers();i++)
			if(is_user_connected(i))
				ColorChat(i,BLUE,"^x04[^x03 Catch VOTE^x04 ]^x01 Yeah:^x03 %d^x04 votes^x01 | Never:^x03 %d^x04 votes",yes,no)
...
public start_vote(id,level,cid) {
	...
	if(enable == true)
		...
		vote = true
		set_task(float(get_cvar_num("amx_vote_time")),"check_vote",3000)
...
public cancel_vote(id,level,cid) {
	...
	if(vote == false)
		ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 There are^x03 no votes^x04 active!")
	else {
		vote = false
		remove_task(3000)
		...
		for(new i=1;i<=get_maxplayers();i++)
			if(is_user_connected(i))
				ColorChat(id,TEAM_COLOR,"^x04[^x03 Catch^x04 ]^x01 Admin^x03 canceled the^x04 Vote!")
	}
