...
#define ADMIN_ELITE		ADMIN_KICK			// Admin ELite Level
#define ADMIN_NADE		ADMIN_IMMUNITY		// Admin Nade Level
...
// Bot name
	...
	new const g_szBotName[ ] = "DeathRun";
#endif
...
public plugin_init( ) {
	new const VERSION[ ] = "3.0.3 rr";
	
	register_plugin( "Deathrun Manager", VERSION, "xPaw / Shidla" );
	...
	g_pGiveUsp       = register_cvar( "deathrun_giveusp",    "0" );
	...
	register_cvar("Shidla", "Deathrun Manager 3.0.3 + Revive REQ + Lives Set & Give Command", FCVAR_SERVER|FCVAR_SPONLY);
...
public EventRoundStart( ) {
	...
	if( iRealPlayers <= 1 ) {
		set_hudmessage( random(255), random(255), random(255), -1.0, 0.1, 0, 4.0, 4.0, 0.5, 0.5, 4 );
...
public Showlifes( id ) {
	set_hudmessage( random(255), random(255), random(255), 0.04, 0.71, 0, 2.5, 2.5, 0.5, 0.5, 3 );
...
public GiveUsp( const id ) {
	if( is_user_alive( id ) )
	{
		if(get_user_flags(id) & ADMIN_ELITE)
		{
