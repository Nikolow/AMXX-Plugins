 ...
 Link to the original plugin
   ...
   httpforums.alliedmods.netshowthread.phpt=78197
...
#define FAKE_PLAYER
 ...
 Bot name
   ...
   new const g_szBotName[ ] = CanndyCore ! MentaK PlayeR;
#endif
...
new const g_szPrefix[ ] = [ DeathRun ];
 ...
 Global Variables
new boolg_bBlockUSP, boolg_bGamerFun, boolg_bRandromized, boolg_bStarting, boolg_bFirstRound;
new boolg_bEnabled, boolg_bRestart, boolg_bConnected[ 33 ];
new MemoryMap[128][64];
new TotalMAPS;

new g_pRemoveBuyZone, g_pHideHuds, g_pBlockMoney, g_pLifeSystem, g_pSvRestart, g_pAutoBalance, g_pLimitTeams, g_ReviveREQ;
...
public GetMAPS()
{
   ...
   format(MapsFile,199,%sdeathrun_blockusp.ini,configsdir);

   if (!file_exists(MapsFile)) 
   {
      server_print([DeathRun] Error Coudn't find %s, MapsFile);
      return PLUGIN_HANDLED;
   }

   NumMaps = file_size(MapsFile,1);
   
   TotalMAPS = 0;
   new temp;
   for(new i=0; iNumMaps; i++)
   {
      read_file(MapsFile,i,MemoryMap[TotalMAPS],63,temp);
      if (!equal(MemoryMap[TotalMAPS],,strlen( MemoryMap[TotalMAPS]))) 
         TotalMAPS++ ;
   }
   server_print([DeathRun] Total %i maps with usp blocking.,TotalMAPS);
...
public plugin_init( ) {
   new const VERSION[ ] = 3.0.3 modified;
   
   register_plugin( Deathrun Manager, VERSION, xPaw  smowin );
   ...
   g_pHideHuds      = register_cvar( deathrun_hidehud,    0 );
   g_pLifeSystem    = register_cvar( deathrun_lifesystem, 2 );
   ...
   g_ReviveREQ       = register_cvar( deathrun_revive_req, 1 );
   ...
   if( contain( szMapName, deathrun_ ) != -1 ) {
      ...
      for(new i=0; iTotalMAPS; i++)
      if(equal( szMapName, MemoryMap[i])) {
         g_bBlockUSP = true;
      }   
   ...
   if( get_pcvar_num( g_pToggle ) ) {
      ...
      register_clcmd( uselife, EventDeath );
...
public plugin_natives()
{
   ...
   register_native( set_life, _set_life, 1 );
   register_native( show_life, _show_life, 1 );
}

public _set_life( id, lifes ) 
{
   g_Lifes[ id ] += lifes;
}

public _show_life(id) 
{
   return g_Lifes[ id ];
}
...
public EventRandromize( ) {
   ...
   new iRandomPlayer, CsTeamsiTeam;
...
public EventRoundStart( ) {
   ...
   new i, iPlayers[ 32 ], iNum, iRealPlayers, CsTeamsiTeam;
   get_players( iPlayers, iNum, c );
...
public CheckTerrorists( ) {
   ...
   new i, iPlayers[ 32 ], iTerrors, iNum, iRealPlayers, CsTeamsiTeam;
   get_players( iPlayers, iNum, c );
...
public EventDeath( ) {
   ...
   if( cs_get_user_team( iVictim ) == CS_TEAM_CT && g_Lifes[ iVictim ]  0 ) {
      ...
      if( iCTcount  1 )
      {
         ...
         {
            new TextRevive[ 555 char ];
            formatex( TextRevive, charsmax( TextRevive ), r%L, iVictim, DR_LIFE_NOW);
            new TextNotRevive[ 555 char ];
            formatex( TextNotRevive, charsmax( TextNotRevive ), r%L, iVictim, DR_LIFE_NOT);
            new TextREQ[ 555 char ];
            formatex( TextREQ, charsmax( TextREQ ), w%L, iVictim, DR_LIFE_COUNT, g_Lifes[ iVictim ]);

            new i_Menu = menu_create(TextREQ, Revive_Munu_REQ);
            menu_additem(i_Menu, TextRevive, 1, 0);
            menu_additem(i_Menu, TextNotRevive, 2, 0);

            menu_display(iVictim, i_Menu, 0);
         }
      }
      else
...
public Revive_Munu_REQ(id, menu, item)
{
   if (item == MENU_EXIT)
   {
      menu_destroy(menu);
...
public FwdHamPlayerSpawn( id ) {
   ...
   if( g_iFakeplayer == id ) {
...
#endif
      new CsTeamsiTeam = cs_get_user_team( id );
      ...
      if( get_pcvar_num( g_pGiveUsp ) && iTeam == CS_TEAM_CT && !g_bBlockUSP )
...
public FwdGameDesc( ) {
   static const GameName[ ] = DeathRun;
...
public FwdHamPlayerDamage( id, idInflictor, idAttacker, FloatflDamage, iDamageBits ) {
...
ColorChat( id, Colortype, const szMessage[], {Float,Sql,Result,_}... ) {
...
CC_ColorSelection( index, type, ColorType ) {
   switch( Type ) {
