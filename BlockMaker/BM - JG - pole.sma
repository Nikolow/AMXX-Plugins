...
#define PLUGIN "JukeGodz Blockmaker"
...
const Float:gfSnapDistance = 10.0;
...
const TASK_SUPERMAN = 12000;

// strings
new const gszPrefix[] = "[ jG ] ";
...
// block models
new const gszBlockModelDefault[] = "models/iGz_HnS_Blockmaker/igz_default.mdl";
...
new const gszBlockModelLowGravity[] = "models/iGz_HnS_Blockmaker/igz_lowgravity.mdl";
...
new const gszBlockModelSlap[] = "models/iGz_HnS_Blockmaker/igz_slap.mdl";
...
new const gszBlockModelBootsOfSpeed[] = "models/iGz_HnS_Blockmaker/igz_boostofspeed.mdl";
new const gszBlockModelGlass[] = "models/iGz_HnS_Blockmaker/igz_platformv2.mdl";
new const gszBlockModelDelayedBhop[] = "models/iGz_HnS_Blockmaker/igz_delay_bhop.mdl";
new const gszBlockModelFade[] = "models/iGz_HnS_Blockmaker/igz_blind.mdl";
...
new const gszBlockModelSmoke[] = "models/iGz_HnS_Blockmaker/igz_frostblock.mdl";
...
new const gszBlockModelTrampolineLow[] = "models/iGz_HnS_Blockmaker/igz_trampoline.mdl";
new const gszBlockModelTrampolineMid[] = "models/iGz_HnS_Blockmaker/igz_trampoline.mdl";
new const gszBlockModelTrampolineHigh[] = "models/iGz_HnS_Blockmaker/igz_trampoline.mdl";
...
new const gszBlockModelSuperman[] = "models/iGz_HnS_Blockmaker/igz_superman.mdl";
...
new const gszBlockNames[gBlockMax][34] =
{
	"Platform",
	"Glass",
	"Bunnyhop",
...
createMenus()
{
	...
	add(gszMainMenu, size, "\y[jG] Block Maker Main Menu^n^n");
	...
	add(gszBlockMenu, size, "\y[jG] Block Menu^n^n");
	...
	add(gszTeleportMenu, size, "\y[jG] Teleporter Menu^n^n");
	...
	add(gszOptionsMenu, size, "\y[jG] Options Menu^n^n");
...
public client_PreThink(id)
{
	...
	if (is_user_connected(id))
	{
		...
		if (isBlock(ent) && (pev(id, pev_button) & IN_USE && !(pev(id, pev_oldbuttons) & IN_USE) ))
		{
			...
			show_hudmessage(id, "Block Type: %s^nCreator: %s", gszBlockNames[blockType], szCreator);
		}
...
actionTrampolineLow(id)
{
	...
	if (halflife_time() >= gfTrampolineLowTimeout[id])
		...
		//set player Z velocity to make player bounce
		entity_get_vector(id, EV_VEC_velocity, velocity);
		velocity[2] = 300.0;					//jump velocity
		entity_set_vector(id, EV_VEC_velocity, velocity);
...
actionAWP(id)
{
	if(is_user_alive(id) && !AwpUsed[id] && get_user_team(id) == 1)
	{
		...
		set_hudmessage(255, 255, 0, -1.0, 0.35, 0, 0.0, 12.0, 0.1, 0.2, 4);
		show_hudmessage(0, "%s has picked up an AWP with 1 bullet!! BEWARE CTS!!", awpname);
