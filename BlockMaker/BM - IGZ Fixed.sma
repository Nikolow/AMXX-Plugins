...
new const SERVERIP[] = "75.102.34.157:27016";
...
new const gszBlockNames[gBlockMax][38] =
{
	"Platform",
	"Bunnyhop",
	"Damage",
...
'A', 'B', 'C', 'D', 'I', 'J', 'H', 'E', 'F', 'K', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', '$', '<', '#', 'Y', 'Z', '!', '@', '=', 'G', ')', '(', '4', '9', '7', '8', '&'
...
createMenus()
{
	...
	add(gszMainMenu, size, "\r3. %s\wNoclip: %s^n");
	add(gszMainMenu, size, "\r4. %s\wGodmode: %s^n^n");
	add(gszMainMenu, size, "\r5. \wOptions Menu^n");
...
actionTrampolineLow(id)
{
	...
	if (halflife_time() >= gfTrampolineLowTimeout[id])
		...
		//set player Z velocity to make player bounce
		entity_get_vector(id, EV_VEC_velocity, velocity);
		velocity[2] = 275.0;					//jump velocity
		entity_set_vector(id, EV_VEC_velocity, velocity);
