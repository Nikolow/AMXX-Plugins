...
new const g_clantag[] = "jG";
new const g_clanrules[] = "jG Rules";
new const g_teleport_start_frames = 10;
...
// bcm set models
new const bcm_platform[] =               "models/RtG/Normal/rtg_platform.mdl";
new const bcm_delayed_bunnyhop[] =	 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_bunnyhop[] =               "models/RtG/Normal/rtg_platform.mdl";
new const bcm_damage[] =                 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_healer[] =                 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_no_fall_damage[] =         "models/RtG/Normal/rtg_platform.mdl";
new const bcm_ice[] =                    "models/RtG/Normal/rtg_platform.mdl";
new const bcm_trampoline[] =             "models/RtG/Normal/rtg_platform.mdl";
new const bcm_speed_boost[] =		 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_death[] =			 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_low_gravity[] =		 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_slap[] =			 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_honey[] =			 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_ct_barrier[] =		 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_t_barrier[] =	         "models/RtG/Normal/rtg_platform.mdl";
new const bcm_glass[] =			 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_no_slow_down_bunnyhop[] =  "models/RtG/Normal/rtg_platform.mdl";
new const bcm_invincibility[] =		 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_stealth[] =		 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_boots_of_speed[] =	"models/RtG/Normal/rtg_platform.mdl";
new const bcm_duck[] =			"models/RtG/Normal/rtg_platform.mdl";
new const bcm_he_grenade[] =		 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_flashbang[] =		 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_smoke_grenade[] =		 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_deagle[] =                 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_awp[] =                    "models/RtG/Normal/rtg_platform.mdl";
new const bcm_blindtrap[] =             "models/RtG/Normal/rtg_platform.mdl";
new const bcm_superman[] =               "models/RtG/Normal/rtg_platform.mdl";
new const bcm_xpblock[] =                "models/RtG/Normal/rtg_platform.mdl";
new const bcm_suffer[] =                 "models/RtG/Normal/rtg_platform.mdl";
new const bcm_poison[] =                "models/RtG/Normal/rtg_platform.mdl";
new const bcm_antidote[] =              "models/RtG/Normal/rtg_platform.mdl";
new const bcm_grenadepick[] =             "models/RtG/Normal/rtg_platform.mdl";
new const bcm_moneygiver[] =             "models/RtG/Normal/rtg_platform.mdl";

new const g_sprite_light[] =		 "models/RtG/Light/light.spr";
...
new g_block_hash_available[TOTAL_BLOCKS];
...
public plugin_init()
{
	...
	register_clcmd("say /bm",		"CmdMainMenu");
	RegisterSayCmd("rtt",			"CmdRockTheConfig");
	RegisterSayCmd("/rrt",			"CmdRockTheConfig");
	RegisterSayCmd("config",		"CmdConfig");
	...
	register_menucmd(register_menuid("BcmTemplateMenu"), 		g_keys_template_menu, 		"HandleTemplateMenu");
	...
	g_cvar_he_grenade_cooldown =	register_cvar("bm_hegrenadecooldown", "60");
	g_cvar_flashbang_cooldown =	register_cvar("bm_flashbangcooldown", "60");
	g_cvar_smoke_grenade_cooldown =	register_cvar("bm_smokegrenadecooldown", "60");
	g_cvar_suffer_cooldown =        register_cvar("bm_suffercooldown", "60");
	jg_money =              register_cvar("moneyblock", "5000");
	register_cvar("bm_respawn", 		  "0");
	register_cvar("BCM_loadingrounds", "0");
	register_cvar("BCM_timelockrtt", "180.0");
	register_cvar("BCM_automaticloading", "0");
	
	new dir[64];
	get_basedir(dir, charsmax(dir));
	add(dir, charsmax(dir), "/tNh");
	...
	formatex(g_config_file, 96, "%s/%s.tNh.config", dir, map);
	for(new x = 0; x < 32; x++) {
		g_rock_the_config[x] = 0;
	}
	for(new x = 0; x < TOTAL_BLOCKS; x++) {
		g_block_hash_available[x] = x;
	}
}
...
public CmdConfigs(id) {
	...
	for(new i = 0; i < g_config_count; i++) {
		BCM_Print(id, "^x01 %d) %s.", g_config_index[i], g_config_names[i]);
	}
}
...
public welcome(id)
{
	...
	ColorChat(id, GREEN, "[%s]^x01 Welcome^x03 %s^x01,  please visit our website at ^x04www.suckmyballs.com!", g_clantag, szPlayerName);
}
...
CreateMenus()
{
	...
	add(g_main_menu, size, "\y[%s] %s v%s^n\yBlockmaker by: \riNfek & Kreation^n\yLoaded Blocks: \r%d^n^n");
	...
	add(g_main_menu, size, "\r6. \wLight Creator^n^n");
	add(g_main_menu, size, "\r7. \wTemplate Menu^n");
	add(g_main_menu, size, "\r8. \wConfig Menu^n");
	add(g_main_menu, size, "\r9. \wOptions Menu^n");
	add(g_main_menu, size, "\r0. \yClose");
	g_keys_main_menu =		B1 | B2 | B3 | B4 | B5 | B6 | B7 | B8 | B9 | B0; 
	
	size = charsmax(g_block_menu);
	add(g_block_menu, size, "\y[%s] Block Menu^n\yLoaded Blocks: \r%d^n^n");
	...
	add(g_teleport_menu, size, "\y[%s] Teleport Menu^n\yLoaded Teleports: \r%d^n^n");
	...
	add(g_light_menu, size, "\y[%s] Light Creator^n\yLoaded Lights: \r%d^n^n");
	...
	add(g_light_properties_menu, size, "\y[%s] Light Settings^n^n");
	...
	add(g_commands_menu, size, "%s3. %sRevive Everyone^n^n");
	add(g_commands_menu, size, "\r0. \yBack");
	g_keys_commands_menu =		B1 | B2 | B3 | B0;
	
	size = sizeof(g_change_team_menu);
	add(g_change_team_menu, size, "\y[%s] Admin Menu^n^n\r");
	...
	add(g_config_menu, size, "\y[%s] Config Menu^n^n");
	add(g_config_menu, size, "\r1. %sLoad Config^n");
	add(g_config_menu, size, "\r2. %sCreate and Load new Config^n");
	add(g_config_menu, size, "\r3. %sDelete current Config \r(\y%s\r)^n");
	add(g_config_menu, size, "\r4. %sRename current Config \r(\y%s\r)^n^n");
	add(g_config_menu, size, "\r0. \wBack");
	...
	g_keys_template_menu =		B1 | B2 | B3 | B4 | B5 | B6 | B7 | B8 | B9 | B0;
...
public EventNewRound(id) {
	...
	BCM_Print(0, "Template loaded is: [^x04%s^x03]", g_config_names[g_current_config]);
	DeleteAll(0, false);
	g_load_start_line = 0;
...
public VoteConfigs() {
	...
	BCM_Print(0, "Voting For a Template Will Start in [^x04 10^x03 ] Seconds");
...
public VoteDone() {
	...
	BCM_Print(0, "Template Voting Finished, Result is: [^x04%s^x03]", g_config_names[g_voted_config]);
...
public CmdRockTheConfig(id) {
	...
	if(g_vote_config_locked) {
		BCM_Print(0, "You Cannot Rock The Template Again!");
		return PLUGIN_HANDLED;
	}
	...
	BCM_Print(0, "Need [^x04%d^x03] More To Rock The Template", floatround(float(connected) * 0.8) - count);
	
	if(floatround(float(connected) * 0.8) <= count) {
		g_vote_config_locked = true;
		set_task(get_cvar_float("bm_timelockrtt" ), "UnlockVoteConfig");
		VoteConfigs();
	}
	return PLUGIN_HANDLED;	
}
...
public CmdConfig(id) 
{
	...
	GetConfigFileName(g_current_config, g_new_file);
	BCM_Print(0, "The Current Config is [^x04%s^x03]", g_config_names[g_current_config]);
...
ActionPoison(id, ent)
{
	...
	{
		BCM_Print(id, "You stepped on a poisonous chemical! Now you need an antidote enjection!", g_is_poisoned[id]);
	}
}
...
ActionAwp(id, ent)
{
	...
	if (is_user_alive(id) && !awpused[id])
	{
		if(team <= 1.0)
		{
			...
			new name[33];
			get_user_name(id, name, 32);
			set_hudmessage(0, 210, 0, -1.0, 0.20, 0, 6.0, 12.0, 0.0, 1.0, 3);
			show_hudmessage(0, "Oh Hell no! %s has picked up a Awp!", name);
		}
		else if(team == 2.0)
		{
			if(get_user_team(id) == 1)
...
ActionDeagle(id, ent)
{
	...
	if (is_user_alive(id) && !deagleused[id])
	{
		if(team <= 1.0)
		{
			...
			get_user_name(id, name, 32);
			set_hudmessage(255, 0, 0, -1.0, 0.20, 0, 6.0, 12.0, 0.0, 1.0, 3);
			show_hudmessage(0, "Omfg! %s has picked up a Deagle!", name);
		}
		else if(team == 2.0)
		{
			if(get_user_team(id) == 1)
...
ActionHEGrenade(id,  ent, OverrideTimer)
{
	...
	new Float:team = str_to_float(property1);
	new Float:fTime = halflife_time();
	if (fTime >= g_he_grenade_next_use[id] || OverrideTimer)
	{
		if(team <= 1.0)
		{
	...
	{
		set_hudmessage(255, 0, 0, gfTextX, gfTextY, gHudEffects, gfHudFxTime, gfHudHoldTime, gfHudFadeInTime, gfHudFadeOutTime, gHudChannel);
		show_hudmessage(id, "HE Grenade next use: %.1f", g_he_grenade_next_use[id] - fTime);
	}
}
ActionFlashbang(id, ent, OverrideTimer)
{
	...
	new Float:team = str_to_float(property1);
	new Float:fTime = halflife_time();
	if (fTime >= g_flashbang_next_use[id] || OverrideTimer)
	{
		if(team <= 1.0)
		{
			...
			new name[33];
			get_user_name(id, name, 32);
			set_hudmessage(120, 120, 120, -1.0, 0.20, 0, 6.0, 12.0, 0.0, 1.0, 3);
	...
	{
		set_hudmessage(120, 120, 120, gfTextX, gfTextY, gHudEffects, gfHudFxTime, gfHudHoldTime, gfHudFadeInTime, gfHudFadeOutTime, gHudChannel);
		show_hudmessage(id, "Flashbang next use: %.1f", g_flashbang_next_use[id] - fTime);
	}
}
...
ActionSmokeGrenade(id, ent, OverrideTimer)
{
	...
	new Float:team = str_to_float(property1);
	new Float:fTime = halflife_time();
	if (fTime >= g_smoke_grenade_next_use[id] || OverrideTimer)
	{
		if(team <= 1.0)
		{
			...
			new name[33];
			get_user_name(id, name, 32);
			set_hudmessage(0, 0, 225, -1.0, 0.20, 0, 6.0, 12.0, 0.0, 1.0, 3);
			show_hudmessage(0, "%s has picked up a Frost Grenade!", name);
		}
		else if(team == 2.0)
		{
			if(get_user_team(id) == 1)
	...
	{
		set_hudmessage(0, 0, 225, gfTextX, gfTextY, gHudEffects, gfHudFxTime, gfHudHoldTime, gfHudFadeInTime, gfHudFadeOutTime, gHudChannel);
		show_hudmessage(id, "Smoke Grenade next use: %.1f", g_smoke_grenade_next_use[id] - fTime);
	}
}
ActionDuck(id)
{
...
ActionHeal(id, ent)
{
	...
	if ( new_health < 100 )	entity_set_float(id, EV_FL_health, new_health);
	else			entity_set_float(id, EV_FL_health, 100.0);
...
{
	new GrenadeMenu = menu_create("\r[RtG] \yPick A Grenade!", "ShowGrenadeMenu");
...
public ShowGrenadeMenu(id, GrenadeMenu, item)
{
	...
	switch(key)
	{
		case 1:// High Explosive
		{
			...
			get_user_name(id, name, 32);
			set_hudmessage(255, 0, 0, -1.0, 0.20, 0, 6.0, 12.0, 0.0, 1.0, 3);
			show_hudmessage(0, "Omfg! %s got a HE-Grenade from Grenade Selection!", name);
		}
		case 3:// Frost Grenade
		{
			...
			new name[33];
			get_user_name(id, name, 32);
			set_hudmessage(168, 230, 29, -1.0, 0.20, 0, 6.0, 12.0, 0.0, 1.0, 3);
			show_hudmessage(0, "Uhoh! %s got a Frost Grenade from Grenade Selection!", name);
		}
		case 4:// Flashbang
		{
			give_item(id, "weapon_flashbang");
			...
			get_user_name(id, name, 32);
			set_hudmessage(168, 230, 29, -1.0, 0.20, 0, 6.0, 12.0, 0.0, 1.0, 3);
			show_hudmessage(0, "Ahhh! %s got a Flash Grenade from Grenade Selection!", name);
		}
	}
	menu_destroy(GrenadeMenu);
	return PLUGIN_HANDLED;
...
ShowConfigMenu(id) {
	...
	col1 = g_admin[id] || g_gived_access[id] ? "\w" : "\d";
	col2 = g_admin[id] || g_gived_access[id] ? "\w" : "\d";
	
	format(menu, charsmax(menu), g_config_menu, PREFIX, col1, col2, col1, g_new_file, col2, g_config_names[g_current_config]);
...
ShowConfigSelectionMenu(id) {
	...
	len += formatex(menu[len], charsmax(menu), "\yConfig Selection Menu (Current file: %s)^n^n", g_new_file);
...
ShowTemplateMenu(id) {
	...
	len += formatex(menu[len], charsmax(menu), "\yTemplate Menu \r(\wCurrent file: \y%s\r)^n^n", g_new_file);
	for(new i = 0; (i < 7) && ((g_config_menu_page[id] * 7 + i) < g_config_count); i++) {
		len += formatex(menu[len], charsmax(menu), "\r%d. \w%s^n", (i + 1), g_config_names[g_config_menu_page[id] * 7 + i]);
		keys |= (1 << i);
	}
	...
	show_menu(id, keys, menu, -1, "BcmTemplateMenu");
}
...
ShowConfigVoteMenu(id) {
	...
	len += formatex(menu[len], charsmax(menu), "\yVote for a config \w[\r20\w]^n^n\yCurrent Config: \w[\r%s\w]^n^n", g_config_names[g_current_config]);
	...
	show_menu(id, keys, menu, 30, "BcmConfigVoteMenu");
}
...
public HandleMainMenu(id, key)
{
	switch ( key )
	{
		...
		case K6: ShowLightMenu(id);
		case K7: ShowTemplateMenu(id);
		case K8: ShowConfigMenu(id);
	...
	switch ( key )
	{
		...
		case K0:
		{
			switch (g_menu_before_options[id])
			{
				...
				default: log_amx("%sPlayer ID: %d has an invalid g_menu_before_options: %d", PREFIX, id, g_menu_before_options[id]);
			}
		}
	}
	...
	switch ( key )
	{
		case K1:
		{
			switch ( g_choice_option[id] )
			{
				...
				default:
				{
					log_amx("%sInvalid choice in handleChoiceMenu()", PREFIX);
				}
			}
		}
		case K2: ShowOptionsMenu(id, 6);
	}
	...
	switch(key) {
		...
		case K4: { 
			if(g_current_config != 0) { 
				...
				BCM_Print(id, "Cannot change the name of the default config.");
				ShowConfigMenu(id);
			}
		}
		case K0: ShowMainMenu(id);
...
public HandleTemplateMenu(id, key) {
	switch(key) {
		case K0: ShowMainMenu(id);
		case K9: g_config_menu_page[id]++;
		case K8: g_config_menu_page[id]--;
...
public HandleConfigVoteMenu(id, key) {
	...
	if(key == K9 || key == K8) {
		...
		BCM_Print(0, "%s voted for ^x03[^x04%s^x03].", name, g_config_names[index]);
	}
}

DeleteCurrentConfiguration(id) {
	if(g_current_config == 0) {
		BCM_Print(id, "Cannot Delete default configuration.");
		return;
	}
	
	BCM_Print(id, "Deleting and loading Default configuration.");
...
public NewConfigName(id) 
{
	...
	{
		BCM_Print(id, "^x03 Max configs of %d has already been reached.", MAX_CONFIGS);
		return PLUGIN_HANDLED;
	}
	new args[32];
...
CmdRevivePlayer(id)
{
	...
	client_cmd(id, "messagemode RtG_Revive");
...
public SetPropertyBlock(id)
{
	...
	&& block_type != BOOTS_OF_SPEED )
	{
		if ( !( 40 <= property_value <= 200
		|| property_value == 255
		|| property_value == 0 ) )
		{
			BCM_Print(id, "The property has to be between^1 40^3 and^1 200^3,^1 255^3 or^1 0^3!");
			return PLUGIN_HANDLED;
		}
	}
	else
...
DeleteAll(id, bool:notify)
{
	...
	static iEntity, block_count, tele_count, light_count, bool:deleted;
	...
	iEntity = -1;
	light_count = 0;
	while ( ( iEntity = find_ent_by_class(iEntity, g_light_classname) ) )
	{
		deleted = DeleteLight(iEntity);
		if ( deleted )
		{
			++light_count;
		}
...
public TaskLoad()
{
	if (file_exists(g_new_file))
	{
		...
		while ( !feof(f) )
		{
			...
			if(count >= g_load_start_line + 20) {
				g_load_start_line = count;
				fclose(f);
				set_task(0.1, "TaskLoad");
				return;
			...
			if ( strlen(type) > 0 )
			{
				...
				switch ( type[0] )
				{
					...
					default:
					{
						log_amx("%sInvalid block type: %c in: %s", PREFIX, type[0], g_file);
...
GetConfigFileName(index, filename[]) {
	...
	get_mapname(map, charsmax(map));
	if(index == 0) {
		formatex(filename, 96, "%s/tNh/%s.tNh", dir, map);
	} else {
		formatex(filename, 96, "%s/tNh/%s.%d.tNh", dir, map, index);
	}
}