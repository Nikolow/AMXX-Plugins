/*
	...
	Версия на плъгина Spec Info, която показва информация за наблюдавания играч. Добавена е информация за Парите му и Кръвта.
...
public tskShowSpec()
{
	...
	for( new alive = 1; alive <= gMaxPlayers; alive++ )
	{
		...
		format(szHud, 45, "Spectate: %s:^n", szName);
		format(szHud, 45, "%s:^nMoney: %d | HP: %d^n^n", szName, cs_get_user_money(alive), get_user_health(alive));
